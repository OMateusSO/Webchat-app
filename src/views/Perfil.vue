  ...
  <div class="flex justify-center min-h-screen px-2 bg-gradient-to-br from-indigo-100 via-white to-indigo-50">
    <div class="max-w-lg w-full">
      ...
      <div class="flex justify-center mb-6">
        <img :src="fotoPerfil || '/src/assets/logo.png'" class="w-64 h-64 rounded-full border-4 object-cover shadow-md">
      </div>
    ...
    <div v-if="showDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        <input v-if="campoAtual === 'nome'" v-model="editValor" placeholder="Digite seu nome" class="w-full p-2 border rounded mb-2">
        <textarea v-if="campoAtual === 'bio'" v-model="editValor" placeholder="Digite sua biografia" class="w-full p-2 border rounded mb-2"></textarea>
        <input v-if="campoAtual === 'foto'" v-model="editValor" placeholder="URL da foto" class="w-full p-2 border rounded mb-2">

        <div class="flex justify-between">
          <button @click="showDialog = false" class="py-2 px-4 bg-gray-300 text-gray-800 font-medium rounded-lg hover:bg-gray-400 transition">
            Cancelar
          </button>
          <button @click="salvarEdicao" class="py-2 px-6 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition">
            Salvar
          </button>
        </div>
      </div>
  ...
  methods: {
    async carregarPerfil() {
      ...
      if (userSnap.exists()) {
        ...
        this.fotoPerfil = userData.fotoPerfil || "";
      }
    },
    abrirDialog(campo) {
